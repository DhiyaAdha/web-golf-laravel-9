...
class OrderController extends Controller
{
        ...
        try {
            if ($cek_itemId->isNotEmpty()) {
                ...
                if($visitor->expired_date <= Carbon::now()) {
                    ...
                    \Cart::session($request->get('member'))->add([
                        'id' => $request->get('package'),
                        'name' => $package->name,
                        'price' => $price,
                        'quantity' => 1,
        ...
        if ($request->get('type') == 'plus') {
            try {
                if ($package->quantity == $cek_itemId[$request->get('id')]->quantity) {
                    ...
                    \Cart::session($request->get('member'))->update($request->get('id'), [
                        'quantity' => [
                            'relative' => true,
                            'value' => 1,
                        ],
    ...
    public function checkout(Request $request, $id)
    {
        ...
        try {
            ...
            $order_number = 'INV/'.Carbon::now()->format('Ymd').'/'.$visitor->tipe_member;
